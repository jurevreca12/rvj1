...
source yosys/yosys_common.tcl

# ABC logic optimization script
set abc_script [processAbcScript yosys/abc-opt.script]

set tmp_dir ./tmp/.
set rep_dir ./rep/.
set out_dir ./output/.

set sv_flist synth.flist
set top_design rvj1_top

# read liberty files and prepare some variables
source yosys/init_tech.tcl
...
yosys setattr -set keep_hierarchy 1 "t:counter"
yosys setattr -set keep_hierarchy 1 "t:register"
...
set abc_comb_script   [processAbcScript yosys/abc-opt.script]
# call ABC
yosys abc {*}$tech_cells_args -D $period_ps -script $abc_comb_script -constr yosys/abc.constr -showtmp
