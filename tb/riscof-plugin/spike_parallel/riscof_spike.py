...
class spike(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            logger.error(
                "Please check the spike_parallel section in config for missing values."
            )
            logger.error(e)
            raise SystemExit
        logger.debug(
            "SPIKE Parallel plugin initialised using the following configuration."
        )
        for entry in config:
    ...
    def build(self, isa_yaml, platform_yaml):
        ...
        compiler = "riscv64-unknown-elf-gcc".format()
    ...
    def runTests(self, testList, cgf_file=None):
        ...
        for file in testList:
            ...
            if cgf_file is not None:
                execute += (
                    self.dut_exe
                    + " --log-commits --log dump --isa={0} +signature={1} +signature-granularity=4 {2};".format(
            ...
            if cgf_file is not None:
                coverage_cmd = (
                    "riscv_isac --verbose error coverage -d \
